...
\begin{abstract}
We investigate whether natural language explanations of grammar rules---which we call \emph{metaexamples}---can accelerate an LLM's learning of that grammar. Using an artificial grammar with no semantic priors, we fine-tune Pythia 1.4B on mixtures of grammar examples and metaexamples via continued pretraining. We find that adding a small amount of metaexamples to the training mix significantly improves the model's ability to produce valid grammar examples at every level of pretraining maturity. This result is consistent with the finding that pretrained LLMs learn a bridge between examples and their explanations \cite{treutlein2024}. We suggest that this bridge is bidirectional; data from either side can help to facilitate learning of the other side.
\end{abstract}
...
In our experiment, example training documents each contain a single valid Tivari3 expression (like those above), while metaexample documents each contain a single natural language sentence about the grammar. The full set of metaexamples (which may be repeated in fine-tuning data):
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item step143000 (${\sim}$300B tokens, fully pretrained)
    \end{itemize}
...
Table~\ref{tab:results} shows validity rates at checkpoint-3000 across all conditions.
...
\bottomrule
\end{tabular}
\caption{Validity rates at checkpoint-3000 across pretraining maturity levels and metaexample ratios. All differences between conditions are statistically significant (two-proportion z-test, $n=20{,}000$ per condition, $p<0.01$).}
\label{tab:results}
\end{table}

\subsection{1\% metaexamples improves grammar learning}

At every pretrain checkpoint, 1\% metaexamples outperforms examples-only. The effect is present at all pretraining maturity levels and is strongest at step1000.
...
\subsection{Less pretraining learns the grammar better}

Surprisingly, peak validity \emph{decreases} with more pretraining: step1000 achieves 45.9\% vs 36.9\% at step143000 (both with 1\% metaexamples).
...
\textbf{Interpretation:} The model cannot learn to generate valid strings from descriptions alone---it needs examples. We need both sides of the bridge to be present to learn it. This differs from the result in \cite{treutlein2024}, but note that their experiments involve well-known concepts like probability, whereas we use a completely novel grammar.
...
We also find that a metaexample fraction of 1\% works better than 10\%, possibly because learning capacity is being spent learning the phrasing of metaexamples rather than learning the structural information they contain. And a metaexample fraction of 100\% makes the model learn nothing about the grammar at all; the complete absence of examples means that the model is unable to connect the learning of metaexamples to examples.

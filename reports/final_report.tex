...
\begin{enumerate}
    ...
    \item \emph{Metaexamples}---descriptions of the underlying structure or rules that govern those phenomena---definitions, explanations, theorems (like the bias of a coin)
\end{enumerate}
...
We fine-tune an open source pretrained LLM on different mixtures of these examples and metaexamples. Our main result is that the presence of metaexamples can in fact accelerate the learning of examples.
...
\caption{Validity rates at checkpoint-3000. Metaexamples 1\% (green) outperforms examples-only (blue) at every pretrain checkpoint, with the largest gap at step1000. Metaexamples 10\% (red) consistently underperforms. The 100\% metaexamples condition (0\% validity everywhere) is omitted.}
\label{fig:results}
\end{figure}

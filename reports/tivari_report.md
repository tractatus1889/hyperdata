...
- **The value of explanations depends on pretraining maturity, revealing three regimes:**
  ...
  - **step36000 (25%) and above:** Examples-only learns nothing (0.0%) — the model has acquired strong priors from pretraining, and a "nonsense" grammar in fine-tuning is not enough to override them. But once hyperdata is added, the model begins to learn the grammar anyway (e.g. hyperdata 10%: 0.3% at step36000, 1.4% at final). This means that by step36000, the model has learned enough to see a relationship between examples and explanations, and the explanations provide enough signal to overcome the model's resistance to the unfamiliar grammar.

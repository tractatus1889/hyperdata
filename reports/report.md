# Metaexamples Experiment: Full Results

## Overview

This report tests whether a language model learns formal grammars more effectively when natural language explanations of the grammar's rules are interleaved with training examples — a technique we call "metaexamples" — compared to training on examples alone. We evaluate across three grammars of increasing complexity.
...
All training datasets learn the grammar at the token-probability level (100% completion accuracy). Metaexamples 5% achieves the best generation validity at 99.2%, outperforming the examples-only baseline (97.6%). Metaexamples 10% has the strongest perplexity discrimination (17.37x ratio).
...
| metaexamples 1% | 20/8,000 | 7,980 | 0.25% |
| metaexamples 5% | 34/8,000 | 7,966 | 0.43% |
| metaexamples 10% | 37/8,000 | 7,963 | 0.46% |
...
| metaexamples 1% | 1.357 | 4.031 | 2.97 |
| metaexamples 5% | 1.360 | 4.054 | 2.98 |
| metaexamples 10% | 1.364 | 4.052 | 2.97 |
...
The examples-only model fails the palindrome_closing test: after `[ A B B`, it assigns P(B)=38.6% vs P(A)=36.8% — it slightly prefers continuing with B rather than closing the palindrome with A. All metaexamples models get this right, with 5% and 10% showing the largest margin (P(A)=40.8-41.8% vs P(B)=32.5-35.5%).
...
| metaexamples 1% | 3,934/6,000 | 2,066 | 65.6% |
| **metaexamples 5%** | **4,012/6,000** | **1,988** | **66.9%** |
| metaexamples 10% | 3,816/6,000 | 2,184 | 63.6% |

Generation validity is moderate (~64-67%) and similar across training datasets. Metaexamples 5% leads slightly at 66.9%, while 10% is the weakest at 63.6%. The palindrome constraint is genuinely difficult — the model must track and mirror content tokens across the full sequence, which requires non-local dependencies that are hard for autoregressive models.
...
| metaexamples 1% | 65.6% | 56.4% | 74.8% |
| metaexamples 5% | 67.5% | 60.6% | 72.5% |
| metaexamples 10% | 64.1% | 56.5% | 70.3% |
...
| metaexamples 1% | 3.494 | 12.054 | 3.45 |
| **metaexamples 5%** | **3.458** | 12.226 | 3.54 |
| metaexamples 10% | 3.509 | 12.248 | 3.49 |

Perplexity ratios are similar (~3.45-3.54). Examples-only and metaexamples 5% are tied for the best discrimination.
...
1. **Metaexamples consistently helps completion tests on complex grammars.** For grammar 3, examples-only achieves 75% while all metaexamples variants achieve 100%. The specific test that benefits — palindrome closing — requires understanding a non-local dependency (mirroring the opening token), exactly the kind of rule that natural language explanations can articulate but that may be hard to extract from examples alone.

2. **Metaexamples 5% tends to produce the best generation quality.** It leads generation validity on grammars 1 and 3 (99.2% and 66.9% respectively). The pattern from grammar 1 holds: too little metaexamples (1%) can hurt, too much (10%) dilutes example-based learning, and 5% is the sweet spot.
...
4. **Perplexity discrimination shows no consistent metaexamples advantage.** PPL ratios are similar across training datasets for grammars 2 and 3. Grammar 1's metaexamples 10% has a slightly higher ratio (17.37 vs ~16.3), but the effect is small.

5. **Grammar complexity matters more than training dataset composition.** The jump from grammar 1 (99.2% validity) to grammar 3 (66.9%) is far larger than any within-grammar difference between training datasets. Metaexamples helps at the margins, but the fundamental difficulty of the grammar dominates.
